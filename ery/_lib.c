...
static void
Protocol_reset_message(ProtocolObject *self)
{
    self->op = OP_KIND;
    ...
    self->id = 0;
    self->route_length = 0;
    // NOTE: this should already be NULL when called
    self->route = NULL;
    self->route_index = 0;
    ...
    self->nframes = 0;
    // TODO: when dynamc, will need to free here
...
{
    return visit(self->msg_callback, arg);
}
